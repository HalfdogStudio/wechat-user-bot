...
function synccheck(wxSession) {
  ...
  return new Promise((resolve, reject)=>{
    ...
    if (wxSession.socketFail >= 6) {
      reject(throw new Error('wxSession socket fail TOO MUCH(maybe network lost)'));
      return;
    }
 ...
 * @param {String} msgId - 消息id
 * @param {Object} wxSession - 微信会话
 * @param {String} voicePath - 图像保存路径
 * @return {Function} 接受wxSession的函数该函数返回包含wxSession的Promise
 */
function webwxgetvoice(msgId, wxSession, voicePath){
  ...
  // 保存图片到文件
  try {
    request.get(voiceUrl, {jar: true}).pipe(fs.createWriteStream(voicePath));
  } catch (e){
    error('下载音频资源失败:', e);
  }
}
